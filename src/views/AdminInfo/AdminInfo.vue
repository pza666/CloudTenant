        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scoped">
            ...
            <el-tooltip effect="dark" content="修改密码" placement="top">
              <el-button icon="el-icon-setting" size="mini" type="warning" @click="addOrEdit('修改密码', scoped.row)"></el-button>
            </el-tooltip>
...
export default {
  ...
  methods: {
    ...
    handleSearch: _.throttle(async function (pnum = 1, key, psize = 2) {
      ...
      try {
        const { data } = await this.$request(
          `admin/search?currentPage=${pageNum}&sizePage=${pageSize}&keyword=${keyword}`
        );
        this.adminData = data.data.records;
        this.total = data.data.total;
        this.message("success", `获取数据成功，共有 ${this.total} 条数据!`);
      } catch (error) {
        this.message("error", "无此用户，请重新查询!");
        this.keyword = "";
        this.getadminInfoList();
      }
    }, 500),
    ...
    async handlestatusChange(adminInfo) {
      ...
      try {
        // 如果权限不够捕获并且提示用户权限不足
        data = await this.$request.post(
          `admin/upStatus?phone=${phone}&status=${status}`
        );
        console.log(data);
      } catch (error) {
        // 如果该管理员的权限不是超级管理员则进行提示，并返回
        this.message("error", "不好意思，您当前权限不足以修改管理员状态!");
        return;
      }
      // 如果符合所有条件并且修改成功提示用户
      if (data.data.code === 200) {
        this.message("success", "修改管理用户权限成功!");
      ...
      this.$refs.passwordInfoRef.validate(async (valid) => {
        if (!valid) {
          ...
          if (oldPasswd !== this.adminData[0].password) {
            ...
            this.message("error", "新密码不能与旧密码重复，请重新输入!");
          } else {
    ...
    deleteAdmin(adminInfo) {
      this.$confirm("此操作将删除该管理员, 是否继续?", "提示", {
        ...
        .catch(() => {
          this.message("error", "不好意思，您的权限不足以删除管理员信息!");
        });
    },
